...
if page == "Overview":
    ...
    col1.metric("Wind Farm", f"{wind_cfg['num_turbines'] * wind_cfg['rated_capacity_mw']:.0f} MW")
    ...
    fig.add_trace(go.Scatter(x=sample.index, y=sample["wind_power_mw"],
                             name="Wind", line=dict(color="#1f77b4")), row=1, col=1)
    fig.add_trace(go.Scatter(x=sample.index, y=sample["solar_power_mw"],
                             name="Solar", line=dict(color="#ff7f0e")), row=2, col=1)
...
# ══════════════════════════════════════════════════════════════════════
    ...
    test_pred = moving_average_forecast(
        data.test.y, data.test.y.shape[1],
    ...
    actual = data.target_scaler.inverse_transform(
        data.test.y[sample_idx].reshape(-1, 2)
    )
    predicted = data.target_scaler.inverse_transform(
        test_pred[sample_idx].reshape(-1, 2)
    )
    ...
    for t, name in enumerate(target_names):
        ...
        fig.add_trace(go.Scatter(x=hours, y=actual[:, t],
                                 name="Actual", mode="lines+markers"))
        fig.add_trace(go.Scatter(x=hours, y=predicted[:, t],
                                 name="Baseline MA", mode="lines",
                                 line=dict(dash="dash")))
        # Confidence bands (from baseline quantiles)
        for q_lo, q_hi, color in [(0.05, 0.95, "rgba(31,119,180,0.15)"),
            ...
            if q_lo in baseline.quantiles and q_hi in baseline.quantiles:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(width=0), name=f"PI {q_lo}-{q_hi}",
                ))
        fig.update_layout(title=f"{name} — 24h Forecast", xaxis_title="Hour Ahead",
...
# ══════════════════════════════════════════════════════════════════════
    ...
    price = np.array([
        CFG["optimizer"]["electricity_peak_price_mwh"] if h in CFG["optimizer"]["peak_hours"]
    ...
    with col2:
        ...
        smart_result = smart.run(
            renewable, renewable * 0.85, renewable * 1.15,
            demand, price,
        )
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Naïve SoC", mode="lines+markers",
    ))
    fig.add_trace(go.Scatter(
        y=[d.battery_soc for d in smart_result.decisions],
        name="Smart SoC", mode="lines+markers",
    ))
    fig.update_layout(title="Battery State of Charge", yaxis_title="SoC",
                      xaxis_title="Hour", height=350)
    st.plotly_chart(fig, use_container_width=True)

    # Curtailment comparison
    fig2 = go.Figure(data=[
        go.Bar(name="Naïve", x=list(range(24)),
               y=[d.curtailed_mw for d in naive_result.decisions]),
        go.Bar(name="Smart", x=list(range(24)),
...
# ══════════════════════════════════════════════════════════════════════
    ...
    smart_res = ForecastDispatch(batt_cfg).run(
        np.clip(ren, 0, 200), np.clip(ren * 0.85, 0, 200),
        np.clip(ren * 1.15, 0, 200), dem, pri,
    )
    curt_red = curtailment_reduction(
        naive_res.total_curtailment_mwh, smart_res.total_curtailment_mwh,
    )
