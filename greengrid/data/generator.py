...
def fetch_real_weather(start_date: str, end_date: str, lat: float, lon: float) -> pd.DataFrame:
    ...
    params = {
        ...
        "end_date": end_date,
        "hourly": ["temperature_2m", "relative_humidity_2m", "surface_pressure", "wind_speed_10m", "wind_direction_10m", "shortwave_radiation", "cloud_cover"],
        "timezone": "auto"
    }
...
def _generate_wind_speed(timestamps: pd.DatetimeIndex, seed: int) -> np.ndarray:
    ...
    diurnal = 1.0 + 0.25 * np.sin(2 * np.pi * (hours - 6) / 24.0)

    # Base Weibull samples
    weibull_shape = 2.0
    raw = rng.weibull(weibull_shape, size=n) * seasonal_scale * diurnal

    # AR(1) smoothing (φ = 0.85) for temporal coherence
    phi = 0.85
    smoothed = np.empty(n)
    smoothed[0] = raw[0]
    for i in range(1, n):
        smoothed[i] = phi * smoothed[i - 1] + (1 - phi) * raw[i]

    # Add gust events (Poisson-like at ~3% rate)
    gusts = rng.poisson(0.03, size=n) * rng.uniform(3, 8, size=n)
...
def _wind_power_curve(
    ...
    # Region II — cubic ramp (physics: power \u221d wind_speed^3)
    mask_ramp = (wind_speed >= cut_in) & (wind_speed < rated_speed)
    power[mask_ramp] = rated_mw * (
        (wind_speed[mask_ramp] - cut_in) / (rated_speed - cut_in)
    ) ** 3
...
def _generate_solar_irradiance(
    ...
    for i in range(1, n):
        cloud[i] = np.clip(
            0.9 * cloud[i - 1] + 0.1 * rng.beta(2, 5), 0, 1
        )
...
def generate_dataset(cfg: dict | None = None) -> pd.DataFrame:
    ...
    logger.info(f"Generating {len(timestamps):,} hourly records "
                f"({tr['start']} to {tr['end']})")
    ...
    temperature = _generate_temperature(
        timestamps, dg["wind"]["location"]["latitude"], seed
    )
    ...
    single_turbine_power = _wind_power_curve(
        wind_speed, wc["rated_capacity_mw"],
        wc["cut_in_speed_ms"], wc["rated_speed_ms"], wc["cut_out_speed_ms"],
    )
    wind_power = single_turbine_power * wc["num_turbines"]

    sc = dg["solar"]
    solar_power = _solar_power(
        ghi, sc["capacity_mw"], sc["panel_efficiency"],
        temperature, sc["temperature_coeff"],
    )
