...
class NaiveDispatch:
    ...
    def run(
        ...
        logger.info(
            ...
            f"total_gen={renewable_mw.sum():.1f}MWh, total_demand={demand_mw.sum():.1f}MWh"
        )
        ...
        for h in range(len(renewable_mw)):
            ...
            decisions.append(DispatchDecision(
                hour=h, renewable_mw=ren, demand_mw=dem, price_mwh=price,
                sold_mw=sold, charged_mw=charged, discharged_mw=discharged,
                curtailed_mw=curtailed, battery_soc=self.battery.soc,
                revenue=revenue,
            ))
...
class ForecastDispatch:
    ...
    def run(
        ...
        charge_score = base_charge + 10 * forecast_confidence  # confidence multiplier
        ...
        base_discharge = np.where(effective_price > 0, effective_price / price_mwh.max() * 100 - surplus_lower, -100)
        discharge_score = base_discharge + 5 * forecast_confidence
        ...
        n_discharge = max(1, H // 4)
        for idx in discharge_priority[:n_discharge]:
            # Only tag for discharge if the score implies it's actually profitable after degradation
            if actions[idx] == 0 and discharge_score[idx] > -50:  # don't override charge decisions
        ...
        for h in range(H):
            ...
            if surplus > 0:
                ...
                # Final check - do not discharge if market price is lower than degradation penalty
                # unless it is a peak hour where grid stability demands it
...
def _aggregate(decisions: list[DispatchDecision]) -> DispatchResult:
    """Summarise a list of hourly decisions."""
    total_rev = sum(d.revenue for d in decisions)
    total_curt = sum(d.curtailed_mw for d in decisions)
    total_sold = sum(d.sold_mw for d in decisions)
    total_charged = sum(d.charged_mw for d in decisions)
    total_discharged = sum(d.discharged_mw for d in decisions)
